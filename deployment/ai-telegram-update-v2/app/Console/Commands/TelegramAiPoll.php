    ...
    public function handle(): int
    {
        ...
        try {
            ...
            foreach ($data['result'] as $update) {
                ...
                if ($text === '/devam') {
                    $cevap = $ai->soruCevapla("Su an devam eden gorevler neler? Kim ne yapiyor?");
                ...
                if ($text === '/stok') {
                    $cevap = $ai->soruCevapla("Kritik stoklar hangileri? Stok durumu nasil?");
                    $telegram->sendMessage($cevap);
                    cache(['telegram_ai_poll_offset' => $updateId], now()->addHours(24));
                    $processed++;
                    continue;
                ...
                if ($text === '/personel') {
                    $cevap = $ai->soruCevapla("Kac personel var? Aktif personel kimler?");
                    $telegram->sendMessage($cevap);
                    cache(['telegram_ai_poll_offset' => $updateId], now()->addHours(24));
                    $processed++;
                    continue;
